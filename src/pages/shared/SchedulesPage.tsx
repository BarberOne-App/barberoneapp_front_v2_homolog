...
export function SchedulesPage() {
  ...
  async function handleSubmitBlockedDate(event: FormEvent) {
    ...
    if (form.recurrenceType === "monthly" && (Number(form.recurrenceMonthDay) < 1 || Number(form.recurrenceMonthDay) > 31)) {
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Aplicado para</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Status</th>
                    ...
                    <tr key={item.id} className="border-b border-border last:border-b-0 hover:bg-secondary/30 transition-colors">
                      ...
                      <td className="px-4 py-3 text-sm text-foreground">
                        ...
                        {item.recurrenceEndDate && item.recurrenceType !== "none" && (
                          <div className="text-xs text-muted-foreground">Ate {formatDate(item.recurrenceEndDate)}</div>
                        )}
                      </td>
                      <td className="px-4 py-3 text-sm text-foreground">
                        {item.clientsOnly ? (
              ...
              <div className="space-y-2">
                ...
                <AppCalendar
                  ...
                  onChange={(date) => setForm((current) => ({
                    ...
                    recurrenceWeekdays: current.recurrenceType === "weekly" ? [date?.getDay() ?? 0] : current.recurrenceWeekdays,
                    recurrenceMonthDay: current.recurrenceType === "monthly" ? String(date?.getDate() ?? 1) : current.recurrenceMonthDay,
                  }))}
            ...
            <div className="space-y-3 rounded-lg border border-border p-3">
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Select value={form.recurrenceType} onValueChange={(value: BlockedDateFormState["recurrenceType"]) =>
                    setForm((current) => ({
                      ...current,
                      recurrenceType: value,
                      recurrenceWeekdays: value === "weekly" && current.recurrenceWeekdays.length === 0
                        ? [dateStringToDate(current.date)?.getDay() ?? 0] : current.recurrenceWeekdays,
                      recurrenceMonthDay: value === "monthly"
                        ? String(dateStringToDate(current.date)?.getDate() ?? 1) : current.recurrenceMonthDay,
                    }))
                  }>
                ...
                {form.recurrenceType !== "none" && (
                  <div className="space-y-2">
                    <Label htmlFor="blocked-recurrence-interval">Repetir a cada</Label>
                    <div className="flex items-center gap-2">
                      <Input id="blocked-recurrence-interval" type="number" min={1} max={365}
                        value={form.recurrenceInterval}
                        onChange={(event) => setForm((current) => ({ ...current, recurrenceInterval: event.target.value }))} />
                      <span className="text-sm text-muted-foreground">
                        {form.recurrenceType === "daily" ? "dia(s)" : form.recurrenceType === "weekly" ? "semana(s)" : "mes(es)"}
                      </span>
                    </div>
                  </div>
                )}
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {RECURRENCE_WEEKDAYS.map((day) => {
                      ...
                      return <Button key={day.value} type="button" size="sm" variant={selected ? "default" : "outline"}
                        onClick={() => setForm((current) => ({ ...current, recurrenceWeekdays: selected
              ...
              {form.recurrenceType === "monthly" && (
                <div className="space-y-2">
                  ...
                  <Input id="blocked-month-day" type="number" min={1} max={31} value={form.recurrenceMonthDay}
                    onChange={(event) => setForm((current) => ({ ...current, recurrenceMonthDay: event.target.value }))} />
                </div>
              )}
              ...
              {form.recurrenceType !== "none" && (
                <div className="space-y-2">
                  ...
                  <AppCalendar value={dateStringToDate(form.recurrenceEndDate)}
                    onChange={(date) => setForm((current) => ({ ...current, recurrenceEndDate: dateToDateString(date) }))}
                    placeholder="Sem data final" fromYear={new Date().getFullYear() - 5}
                    toYear={new Date().getFullYear() + 10} className="w-full" />
                  {form.recurrenceEndDate && <Button type="button" variant="ghost" size="sm"
                    onClick={() => setForm((current) => ({ ...current, recurrenceEndDate: "" }))}>Remover data final</Button>}
                </div>
              )}
            </div>
